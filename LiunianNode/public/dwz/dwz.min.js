		...
		theme: function(options){
			...
			return this.each(function(){
				...
				var setTheme = function(themeName){
					...
					window.localStorage.setItem("dwz_theme", themeName);
				...
				if ($.isFunction($.cookie)){
				    ...
				    if (!themeName) {
				        themeName = window.localStorage.getItem("dwz_theme");
				    }
					if (themeName) {
						setTheme(themeName);
					} 
...
$.fn.extend({
	...
	dwzExport: function(){
	    function _doExport($this) {
			...
			var url = unescape($this.attr("href")).replaceTmById($this.parents(".unitBox:first"));
...
(function($){
	...
	var _onchange = function (event) {
		...
		var pId = event.data.$this.attr("value");
        //新加
		if (pId && pId > 0) {
		    var data = {};
		    var $ids = $ref.attr("data");
		    if ($ids) {
		        var arry = $ids.split(',');
		        for (var i = 0; i < arry.length; i++) {
		            if (arry[i]) {
		                data[arry[i]] = $("select[name='" + arry[i] + "']").val();
		            }
		        }
		    }
		    //新加end
